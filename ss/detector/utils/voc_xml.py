...
def save_xml(img_path, save_path, object_list, zoom_rate=1):
    path = os.path.normpath(img_path)
    path_splt = path.split(os.sep)
    # file_prefix = '/data/tuan-anh/aicr-TODAH/test_images/vn_test/Anno/' + path_splt[-1][:-4]
    ...
    for obj in object_list:
        # print(obj)
        xmin = int(round(obj[0]/zoom_rate))
        ymin = int(round(obj[1]/zoom_rate))
        xmax = int(round(obj[2]/zoom_rate))
        ymax = int(round(obj[3]/zoom_rate))
        # print()
        # writer.addBndBox(xmin, ymin, xmax, ymax, list(CLASS_MAPPING.keys())[list(CLASS_MAPPING.values()).index(obj[4])], difficult)
        # if obj[4] != '':
        ...
        #     writer.addBndBox(xmin, ymin, xmax, ymax, obj[4], difficult)
        if obj[6] == 'vietnam':
            writer.addBndBox(xmin, ymin, xmax, ymax, 'ạ', difficult)
        elif obj[6] == 'alphabet':
            writer.addBndBox(xmin, ymin, xmax, ymax, 'b', difficult)
        elif obj[6] == 'symbol':
            writer.addBndBox(xmin, ymin, xmax, ymax, '.', difficult)
        elif obj[6] == 'number':
            writer.addBndBox(xmin, ymin, xmax, ymax, '1', difficult)

        # print(list(CLASS_MAPPING.keys())[list(CLASS_MAPPING.values()).index(obj[4])])
        # writer.addBndBox(113, 40, 450, 403, 'face', difficult)
    save_path = os.path.join(save_path, '{}.xml'.format(file_prefix))
    # writer.save('/data/tuan-anh/aicr-TODAH/test_images/vn_test/Anno/{}.xml'.format(file_prefix))
    writer.save(save_path)


def save_txt(img_path, save_path, object_list, zoom_rate=1):
    path = os.path.normpath(img_path)
    path_splt = path.split(os.sep)
    file_prefix = path_splt[-1][:-4]
    img = Image.open(img_path)
