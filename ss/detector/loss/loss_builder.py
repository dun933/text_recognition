...
class MultiboxLoss(object):
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        total_loss = ((1 - self.alpha) * total_loss) + (self.alpha * pos_loc_loss) / num_pos
        return total_loss


class MultiboxFocalLoss(object):
    """Multibox loss with some helper functions.
    ...
    def _l1_smooth_loss(self, y_true, y_pred):
        smooth = focal.smooth_l1()
        regression_loss = smooth._smooth_l1(y_true, y_pred)
        return regression_loss

    def _focal_loss(self, y_true, y_pred):
        fc = focal.focal()
        fc_loss = fc._focal(y_true, y_pred)
        return fc_loss
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        num_pos = tf.where(tf.not_equal(num_pos, 0), num_pos,
                           tf.ones_like(num_pos))
        # total_loss += (self.alpha * pos_loc_loss) / num_pos
        total_loss = (total_loss * (1 - self.alpha)) + ((self.alpha * pos_loc_loss) / num_pos)
        return total_loss
