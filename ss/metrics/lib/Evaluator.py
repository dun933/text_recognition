...
class Evaluator:
    def GetDetectorTruePositive(self, boundingboxes):
        groundTruths = []
        detections = []
        ...
        TP_gt = np.zeros(len(groundTruths))
        FN_gt = np.zeros(len(groundTruths))

        for g in range(len(groundTruths)):
            pd = [pd for pd in detections if pd[0] == groundTruths[g][0]]
            # print(len(pd))
            for j in range(len(pd)):
                (preds_x, preds_y) = pd[j][3]
                (xmin, ymin, xmax, ymax) = groundTruths[g][3]
    ...
    def GetF1ScoreMetrics(self, boundingboxes):
        ...
        FP_wrong_label = np.zeros(len(dects))
        ...
        for d in range(len(dects)):
            ...
            for j in range(len(gt)):
                ...
                if (xmin <= preds_x) & (xmax >= preds_x) & (ymin <= preds_y) & (ymax >= preds_y) & (
                        preds_label != label):
                    FP_wrong_label[d] = 1
        ...
        r = {
            ...
            'FP': np.sum(FP),
            'FN': np.sum(FN),
            'FP_wrong_label': np.sum(FP_wrong_label),
            'Detections': len(dects),
            'Ground truth': len(gts)
        }
        # ret.append(r)
        return r
    ...
    def GetTruePositive(self, boundingboxes):
        ...
        TP_gt_vn = np.zeros(len(vn_gt_chars))
        FN_gt_vn = np.zeros(len(vn_gt_chars))
        ...
        for g in range(len(vn_gt_chars)):
            pd = [pd for pd in dects if pd[0] == vn_gt_chars[g][0]]
            for j in range(len(pd)):
                # cal FN
                (preds_x, preds_y) = pd[j][3]
                (xmin, ymin, xmax, ymax) = vn_gt_chars[g][3]
                preds_label = pd[j][1]
                label = vn_gt_chars[g][1]
                matching = (xmin < preds_x) & (xmax > preds_x) & (ymin < preds_y) & (ymax > preds_y) & (
                        preds_label == label)
                if matching:
                    TP_gt_vn[g] = 1
