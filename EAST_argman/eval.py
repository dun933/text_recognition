...
def detect(score_map, geo_map, timer, score_map_thresh=0.5, box_thresh=0.1, nms_thres=0.45):
    '''
    ...
    for i, box in enumerate(boxes):
        ...
        cv2.fillPoly(mask, box[:8].reshape((-1, 4, 2)).astype(np.int32) // 4, 2)
                ...
                if boxes is not None:
                    ...
                    with open(res_file, 'w') as f:
                        for idx, box in enumerate(boxes):
                            ...
                            if save_crop:
                                crop = crop_from_img_rectangle(im, box[0, 0], box[0, 1], box[1, 0], box[2, 1])
