...
class YOLOWriter:
    ...
    def BndBox2txt(self, box, classList=[]):
        xmin = box['xmin']
        xmax = box['xmax']
        ymin = box['ymin']
        ymax = box['ymax']
    ...
    def save(self, classList=[], targetFile=None):
        ...
        if targetFile is None:
            ...
            out_class_file = codecs.open(classesFile, 'w', encoding=ENCODE_METHOD)
    ...
    def save_txt(self, classList=[], targetFile=None):
        ...
        for box in self.boxlist:
            boxName, xcen, ycen, w, h = self.BndBox2txt(box, classList)
            # print (classIndex, xcen, ycen, w, h)
            out_file.write("%s %d %d %d %d\n" % (boxName, xcen, ycen, w, h))
...
class YoloReader:
    ...
    def __init__(self, filepath, image, classListPath=None):
        ...
        classesFile = codecs.open(self.classListPath, 'r', encoding='utf8')
    ...
    def addShape(self, label, xmin, ymin, xmax, ymax, difficult):
        # xmin = float(xmin)/self.imgSize[1]
        # xmax = xmin + float(xmax)/self.imgSize[1]
        # ymin = float(ymin)/self.imgSize[0]
        # ymax = ymin + float(ymax)/self.imgSize[0]
        xmax += xmin
        ymax += ymin
    ...
    def parseYoloFormat(self):
        bndBoxFile = codecs.open(self.filepath, 'r', encoding='utf8')
        ...
        for bndBox in bndBoxFile:
            ...
            self.addShape(classIndex, float(xcen), float(ycen), float(w), float(h), False)
