...
class SymSpell(object):
    ...
    def create_dictionary_entry(self, key, count):
        ...
        for delete in edits:
            if delete not in self._deletes:
                self._deletes[delete] = []
    ...
    def create_bigram_dictionary(self, corpus, encoding=None):
        if not os.path.exists(corpus):
            return False
        with open(corpus, "r", encoding=encoding) as infile:
            for line in infile:
    ...
    def load_dictionary(self, corpus, term_index, count_index,
        ...
        **NOTE**: Merges with any dictionary data already loaded.
        ...
        return True
    def load_dictionary_from_list(self, corpus, term_index, count_index):
        if not len(corpus):
            return False
        for line in corpus:
            if len(line) >= 2:
                key = line[term_index]
                count = helpers.try_parse_int64(line[count_index])
                if count is not None:
                    self.create_dictionary_entry(key, count)
        return True

    def load_bigram_dictionary_from_list(self, corpus, term_index, count_index):
        if not len(corpus):
            return False
        # with open(corpus, "r", encoding=encoding) as infile:
        for line in corpus:
    ...
    def lookup(self, phrase, verbosity, max_edit_distance=None,
        ...
        #         print('dhkalsjhdlashd',lis,  similar(lis, 'nội'))
                # print('init', lis)
        # print('init', self._deletes.__getitem__('ội'))
        while candidate_pointer < len(candidates):
            ...
            if candidate in self._deletes:
                ...
                # print('dict_suggestions: ', dict_suggestions)
                for suggestion in dict_suggestions:
                    ...
                    # (suggestionLen >= prefixLength)
                        ...
                        if (self._prefix_length - max_edit_distance == candidate_len
                                and (min_distance > 1
                                     and phrase[phrase_len + 1 - min_distance:] != suggestion[
                                                                                   suggestion_len + 1 - min_distance:])
                                or (min_distance > 0
            ...
            if (len_diff < max_edit_distance
                ...
                if (verbosity != Verbosity.ALL
                    ...
                    continue
                # print('self._deletes', self._deletes)
                for i in range(candidate_len):
                    delete = candidate[: i] + candidate[i + 1:]
                    # simi_candi = dict()
                    sorted_dict = {}
                    # if delete == 'ội':
                    #     print('case   ',self._deletes.get('ội'))
                    # if delete in self._deletes:
                    #     for target in self._deletes[delete]:
                    #         simi_candi[target] = similar(target, delete)
                        # sorted_dict = sorted(simi_candi.items(), key=lambda kv: (kv[1], kv[0]))
                        # sorted_dict = {k: v for k, v in sorted(simi_candi.items(), key=lambda item: item[1])}
                        # sorted_dict = collections.OrderedDict(simi_candi)
                    # print(delete, sorted_dict)
                    # print('delete :', delete)
            ...
            for i in range(1, imax + 1):
                ...
                # pylint: disable=C0301,R0916
                      ...
                      or ((compositions[idx].distance_sum + top_ed == compositions[dest].distance_sum
                           or compositions[idx].distance_sum + separator_len + top_ed == compositions[
                               dest].distance_sum)
    ...
    def _edits(self, word, edit_distance, delete_words):
        ...
        if word_len > 1:
            for i in range(word_len):
                ...
                # print(delete)
                # delete = re.sub(r'[^\x00-\x7F\x80-\xFF\u0100-\u017F\u0180-\u024F\u1E00-\u1EFF]', u'', delete)
